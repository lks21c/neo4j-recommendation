...
rating = pd.read_csv(train_data, sep='\t' ,header=None, names=rating_col)
n_r = rating.shape[0]
...
for m,row in movie.iterrows() :
    ...
    for g in related_genres :
        ...
        tx.append(statement2,\
                  {"A": row.loc['id'], "B": row.loc['title'].decode('latin-1'), "C": row.loc['IMDb url'], "D": np.asscalar(g)})
...
for r,row in rating.iterrows() :
    ...
    tx.append(statement, {"A": np.asscalar(row.loc['user_id']), "B": np.asscalar(row.loc['rating']), "C": np.asscalar(row.loc['item_id'])})
    if r % 100 == 0:
        tx.process()
